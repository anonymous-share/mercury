...
//			varDictionary.put(ID, b);
	...
	protected double runSolver(int choice){
		...
		double sol[] = new double[ID];
			...
			Utils.printLogWithTime("Finish solving with Gurobi.");
...
//			System.out.println("Shut off presolve.");
//			env.set(GRB.IntParam.PrePasses, 1);
...
//			Utils.printLogWithTime("Start optimizing with heuritics.");
			if(Configuration.performTune){
				Utils.printLogWithTime("=================Start tuning!================");
				model.tune();
				model.getTuneResult(0);
				Utils.printLogWithTime("=================End tuning!==================");
				int rand = (int)(Math.random()*10000);
				File paramOut = new File("./param"+rand+".prm");
				Utils.printLogWithTime("Write tuned parameters to "+paramOut.getAbsolutePath());
				model.write(paramOut.getAbsolutePath());
			}
			model.optimize();
...
//			model.optimize();
	...
	public double computeGuiOffloadedTime() {
		double ret = 0.0;
		for(Method t : program.getUIThreads()){
			ret += this.calculateOffloadedTimeRecursively(t);
		}
		return ret;
	}

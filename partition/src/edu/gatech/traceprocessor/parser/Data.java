...
public class Data {
	...
	public static String extractClassType(String s){
		//([Ljava/lang/Object;)Ljava/util/ArrayList;.array
		
		int l = s.indexOf(')');
		int r = s.lastIndexOf(';');

		return r<=l ? s : s.substring(l+1, r);
	}
	
	public Data(long addr, int size, int index, String _typeName) {
		super();
		this.addr = addr;
		this.size = size;
		this.typeName = extractClassType(_typeName);
	...
	public String toString() {
		return "Data [addr=" + addr + ", size=" + size + ", index=" + index +", type=" + typeName+ "]";
	}

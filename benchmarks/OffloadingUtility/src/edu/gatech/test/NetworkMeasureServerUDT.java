...
public class NetworkMeasureServerUDT implements Runnable {
	...
	public NetworkMeasureServerUDT(){
	...
	public void run() {
		...
		try {
			...
			UDTServerSocket resultSocket=new UDTServerSocket(InetAddress.getByName("localhost"), 9220);
			Log.d(TAG, "Now NetworkMeasureServer is listening on 9220");
			
			//Socket inSocket = resultSocket.accept();
			UDTSocket inSocket = resultSocket.accept();
...
//				out = inSocket.getOutputStream();
			...
			Log.d(TAG, "Error happen on listen/accept port: 9220.");
			e.printStackTrace();
		}
		catch(InterruptedException ie){
			Log.d(TAG, "UDT throw exception when listen/accept port: 9220.");
			ie.printStackTrace();
		}
	}

...
import edu.gatech.mcc.objectTracker.MCCFactoryTrackerObjectQuad;
import edu.gatech.mcc.objectTracker.MCCFactoryTrackerObjectQuad_UDP;
		...
		implements AdapterView.OnItemSelectedListener, View.OnTouchListener
{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(!this.hasInit ){
			DemoMain.logger.println("Mosaic "+Protocol.ifOffload+" "+Protocol.ifStateful);
			this.hasInit = true;
		}

//		Utility.logPrefix = "ObjectTracker ";
		MCCFactoryTrackerObjectQuad.res = this.getAssets();
		MCCFactoryTrackerObjectQuad_UDP.res = this.getAssets();
...
//		Utility.logPrefix = "ObjectTracker ";
	...
	private void startObjectTracking(int pos) {
		...
		switch (pos) {
			case 0:
				...
				tracker = MCCFactoryTrackerObjectQuad.circulant(null, ImageUInt8.class);
				//tracker = MCCFactoryTrackerObjectQuad_UDP.circulant(null, ImageUInt8.class);
	...
	protected class TrackingProcessing<T extends ImageBase> extends VideoImageProcessing<MultiSpectral<ImageUInt8>>
	{
		...
		protected void process(MultiSpectral<ImageUInt8> input, Bitmap output, byte[] storage)
		{
			...
			if(counter == 100){
				long totalTime = System.nanoTime() - nstart;
				DemoMain.logger.println("Total time: "+totalTime/Utility.NANO_TO_MILI);
				DemoMain.logger.println("FPS: "+(100.0)*Utility.NANO_TO_MILI*1000/totalTime);
				DemoMain.logger.flush();
				System.exit(0);	
			}
			...
			synchronized(DemoMain.class){
				if(this.hasStarted){
					++counter;
					DemoMain.logger.println("Loop time: "+(end-start)/Utility.NANO_TO_MILI);
					DemoMain.logger.flush();
				}
			}
		}

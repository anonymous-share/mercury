...
/*
 * Stateful uni-directional with piecewise linear network model
 * TODO: Fix latency computation, charge twice when a function is offloaded
 */
public class ILPOffloadingAlgorithm3a extends ILPOffloadingAlgorithm {
	...
	public ILPOffloadingAlgorithm3a(Function forest, Set<VarEntry> programVarTable, Set<Set<Function>> coLocSCCs) {
	...
	//	for(Function func : funcMap.keySet()){
		...
		//	SFunction extFunc = funcMap.get(func);
			...
			{						
				...
				consL2.add(new Pair<Double, BooleanVariable>(1.0, pi[0]));
				ILPConstraint cons2 = new ILPConstraint(consL2, 1, 2);
				...
				consL3.add(new Pair<Double, BooleanVariable>(-1.0, di));
	...
	//		double dataWeight = Configuration.getTransportTime(v);
		...
		//	varFuncIDs.add(varWriter.getId());
			...
			consL1.add(new Pair<Double, BooleanVariable>(1.0, varWriter.getSolverVar(L)));
					if(varWriter.getSolverVar(L) == null)

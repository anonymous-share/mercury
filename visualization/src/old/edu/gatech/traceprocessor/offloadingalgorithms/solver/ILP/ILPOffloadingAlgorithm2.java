...
public class ILPOffloadingAlgorithm2 extends ILPOffloadingAlgorithm {
	...
	public ILPOffloadingAlgorithm2(Function forest, Set<VarEntry> programVarTable, Set<Set<Function>> coLocSCCs) {
...
//		Utils.printLog("CumulativeDataStatsGenerated");
	...
	//		SFunction extFunc = funcMap.get(func);
			...
			BooleanVariable pi = addSolverVar(extFunc, "P", dataWeight, P);
	
			BooleanVariable qi = addSolverVar(extFunc, "Q", 0, Q);
			...
			//Pi => Qi
			addImpliesConstraint(pi, qi);
			
			//Qi => Li
			addImpliesConstraint(qi, li);
			...
			//	assert extParentFunc!=null;
				...
				BooleanVariable pQi = addSolverVar(extParentFunc, "Q", Q);
				
				//Constraint that Qj => Qi where j is the parent node of i
				addImpliesConstraint(pQi, qi);
